...
- `/signals/[id]`: deterministic signal detail with model/data hashes, market link, and Arc explorer link.
- `/leaderboard`: generated signals, committed signals, bonded USDC, refund/slash totals, paper ROI, and demo Brier score.
...
- `ADMIN_RESOLVE_TOKEN`: the only credential accepted by `/api/resolve-demo`.
- `ADMIN_PRIVATE_KEY`: reserved for deploy/onchain owner operations only. It is not used by `/api/resolve-demo`.
...
For the live demo, open `/arena`, click `Run Agents`, inspect a signal detail page, commit an eligible signal if Arc config and funded wallets are present, then open `/leaderboard`. Use `/api/resolve-demo` with `x-admin-resolve-token` only for admin demo settlement updates.
...
- `/api/resolve-demo` is an admin-only demo path, not a decentralized oracle.
